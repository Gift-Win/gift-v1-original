...
interface IGiftV1 {
  ...
  event NewHippyFee(uint256 _fee);
  event Mutiny(address _admin);
  ...
  event NewKing(address _hippyKing);
  event Shame(address _cowardKing);
  event Hippy(string _hi_pi_pi);
  ...
  function setHippyFee(uint256 _fee) external;
  ...
  function crownMyself() external payable;

  function abdicateCrown(string memory _plea) external;

  function decree() external;
